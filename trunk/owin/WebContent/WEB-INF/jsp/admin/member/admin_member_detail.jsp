...
<SCRIPT LANGUAGE="JavaScript">
	function checkIT() {
		...
		var http = new JKL.ParseXML("adminMember.php", query);
...
function secedeMember()
{
	if (confirm("<%=member.getUserId()%>(<%=member.getNickName()%>) 회원을\n탈퇴시키시겠습니까?")) {
		...
		alert(result.resultXml.message);
		if (result.resultXml.code == 0)
			location.href = "adminMember.php?mode=viewAdminMember&status=US001";
	}	
}
...
function plus_charging()
{
	...
	if (confirm("<%=member.getNickName()%>님께 " + f.plus.value + " 원을 직충전 하시겠습니까?")) {
		...
		var http = new JKL.ParseXML("adminAccount.php", query);
		var result = http.parse();
		alert(result.resultXml.message);
		location.reload();
	}
