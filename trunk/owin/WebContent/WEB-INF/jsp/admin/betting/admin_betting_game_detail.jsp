...
<form name='search' method='get' action='adminBetting.php'>
...
%>
	...
	<tr align='center' bgcolor='#ffffff'>
		...
		<td rowspan='<%=betGameCount%>'><font color='#000000'><a href='javascript:goMemberDetail("<%=betting.getUserId()%>")'><%=betting.getUserId()%></a> (<%=betting.getNickName()%>)</a></td>
		<%
		}
		%>
...
function cancelBetting(id)
{
	if (confirm("취소하시겠습니까?")) {
		...
		query += "&id=" + id;
		var http = new JKL.ParseXML("adminBetting.php", query);
		var result = http.parse();
		alert(result.resultXml.message);
		if (result.resultXml.code == 0)
			location.href='adminBetting.php?mode=viewBettingMoneyDetail&id=<%=id%>';
	}
}
</script>
