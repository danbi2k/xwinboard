...
<tr><td width="50%" height="30">
        ...
        <a href="board.php?mode=viewBoard&boardName=<%=boardName%>&pageIndex=<%=pageIndex%>"><img src="images/btn_list.gif" border=0></a>
		<!--
        &nbsp;&nbsp;
...
function deleteItem()
{
	if (confirm("삭제하시겠습니까?")) {
		...
		var http = new JKL.ParseXML("board.php", query);
		var result = http.parse();
		alert(result.resultXml.message);
		if (result.resultXml.code == 0) {
			location.href = "board.php?mode=viewBoard&boardName=<%=boardItem.getBoardName()%>&grade=<%=boardItem.getGrade()%>";
		}
	}
}
