					...
					<tr height="30" bgcolor="#e0e0e0" style="padding-left:30pt">
						<td>
							<select name=KeyField>
								...
								<option value="Writer">작성장</option>
							...
							<select name=pageSize onchange="MovePage('','InList.asp','','',this.value,'');">
								<option <%=gSelection(pageSize,10,"selected")%> value="10">10 개씩</option>
								<option <%=gSelection(pageSize,20,"selected")%> value="20">20 개씩</option>
								<option <%=gSelection(pageSize,50,"selected")%> value="50">50 개씩</option>
								<option <%=gSelection(pageSize,80,"selected")%> value="80">80 개씩</option>
								<option <%=gSelection(pageSize,100,"selected")%> value="100">100 개씩</option>
								<option <%=gSelection(pageSize,150,"selected")%> value="150">150 개씩</option>
								<option <%=gSelection(pageSize,200,"selected")%> value="200">200 개씩</option>
							</select>보기
								&nbsp;&nbsp;&nbsp;
								<input type=radio value=1 onclick="ContentViewFlag(1)")펼침보기
								<input type=radio value=0 onclick="ContentViewFlag(0)")일반보기
						</td>
					</tr>
					</form>
				</table>
			...
			<td align=center><br>
				<table width="790" border="0" cellspacing="1" cellpadding="0" bgcolor="#aaaaaa">
					...
					<tr height="30" align="center" bgcolor="#e0e0e0">
						...
						<td width="80">번 호</td>
						<td width="800"><%If SortColumn="Subject" And strSort = "DESC" Then %><a href="javascript:ReSortList('Subject','ASC');"><font color=#EEEEEE>▲<%ElseIf SortColumn="Codes" And strSort = "ASC" Then%><a href="javascript:ReSortList('Subject','DESC');"><font color=#EEEEEE>▼<%Else%><a href="javascript:ReSortList('Subject','DESC');"><%End If%>제 목</td>
						<!--<td width="150">아이디</td>-->
