...
If strFlag	= "MODIFY" or strFlag = "REPLY" Then
	...
	If objRs.BOF or objRs.EOF Then Call descment("잘못된 요청입니다.","","")
		...
		If strFlag = "REPLY"  Then
			RunSQL("UPDATE "& TableName &"  Set PRef=1 WHERE Ref='"& IntIdx &"'")
			strSubject		= "[RE]:" &  " " & strSubject
			strRefContent	= reString(objRs("content"),"text")
			strContent		= ""
			strMail			= ""	: strHomePage	= ""
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
		...
		<tr>
			<td align=center><br>
				...
				<% If Sect=3 Then %><input type="hidden" name=Idd			value="<%=strId%>"><%End If%>
				
					<tr >
						<td width="16%" bgcolor="#e0e0e0">&nbsp;&nbsp;작성자</td>
						<td bgcolor="#FFFFFF"><input type=text name=writer size=30 MaxBytes="20" value="<%=strWriter%>" Information="Required" title="작성자" class=text></td>
					</tr>
					<tr >
...
%>	
					<tr >
						<td width="16%" bgcolor="#e0e0e0">&nbsp;&nbsp;원문</td>
						<td bgcolor="#FFFFFF"><%=strRefContent%></td>
					</tr>
<%
					End If
%>					
