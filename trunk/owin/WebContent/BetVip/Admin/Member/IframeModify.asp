...
<%
	...
	If Flag = "MODIFY" Then
		...
		Call CloseDB()
		Call descment("수정되었습니다.","IframeModify.asp?Id="&strId&"&CurPage="&CurPage&"&CurBlock="&CurBlock&"&KeyField="&KeyField&"&KeyWord="&KeyWord&"&Sect="&Sect ,"")
	End If
		...
		<tr>
			<td><br>
				<table width="790" border="0" cellspacing="1" cellpadding="3" bgcolor="#aaaaaa">
					...
					<form name=frms method=post action="IframeModify.asp" onsubmit="return DefaultFormValidation(this);" >
			...
			<td align=center><br>
				<table border="0" cellspacing="1" cellpadding="0" >
					...
					<tr height="30" >
						...
						<td ><input type=button value="중 지" onclick="MovePage('<%=strId%>','IframeModify.asp','MODIFYSC',1,'');" ></td>
						<td ><input type=button value="중지해제" onclick="MovePage('<%=strId%>','IframeModify.asp','MODIFYSC',0,'');"  id=button2 name=button2></td>
						<td ><input type=button value="삭 제" onclick="if(confirm('삭제된 회원은 복구할 수 없습니다.\n\n삭제하시겠습니까?')){MovePage('<%=strId%>','IframeModify.asp','DELETE','','');}" id=button1 name=button1></td>
						<td ><input type=button value="닫 기" onclick="self.close()" id=button3 name=button3></td>
					</tr>
				</form>
				</table>
			</td>
