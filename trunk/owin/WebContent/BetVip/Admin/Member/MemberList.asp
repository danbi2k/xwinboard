					...
					<tr height="30" bgcolor="#e0e0e0" style="padding-left:30pt">
						<td>
							<select name=KeyField>
								...
								<option value="Name" <%=gSelection(KeyField,"Name","selected")%>>이 름</option>
...
%>
					<tr height="20" align="center" bgcolor="#FFFFFF" onMouseOver='this.style.backgroundColor="#e4e4e4"' onMouseOut='this.style.backgroundColor=""'>
						...
						<td><nobr>
							<input type=button value="충전내역" onclick="MM_winopen('MCList.asp?Sect=0&Id=<%=objRs("Id")%>','MG','width=820,height=700');">
							<input type=button value="환전내역" onclick="MM_winopen('MCList.asp?Sect=1&Id=<%=objRs("Id")%>','MG','width=820,height=700');" id=button1 name=button1>
							<input type=button value="중 지" onclick="MovePage('<%=objRs("Id")%>','MemberList.asp','STOP','','');">
							<input type=button value="삭 제" onclick="if(confirm('삭제 하시겠습니까?')) {MovePage('<%=objRs("Id")%>','MemberList.asp','DELETE','','');}" >
						</td>
					</tr>
<%
					       gSeq = gSeq - 1
