...
SQL = "SELECT convert(varchar(10),WriteDay,120 ) As WriteDay FROM BetGame_TB WHERE VisbledDBN=0 "
...
'					    Response.Write SQLs&":P<BR>"
					    ...
					    SQLs = "SELECT SUM(ResultCash) FROM Cash_TB WHERE Idx IN (SELECT max(idx) FROM Cash_TB WHERE Status=1 Group By Id)"
'					    Response.Write SQLs&":AP<BR>"
					    Set Rss = RunSQLReturnRs(SQLs)
					    If Not Rss.EOF Then ToAPoint = Rss(0) : Set Rss = Nothing
					    
					    If DateDiff("d",Now(),objRs("WriteDay"))=0 Then	ToNPoint = ToAPoint
'					    TocMoney = TobMondy-TorMondy
						ToSumMoney	= reInt(ToNrMondy,0)-reInt(TorMondy,0)
...
<%
							allTobMoney		= allTobMoney + TobMondy
							allTorMoney		= allTorMoney + TorMondy
							allToSumMoney	= allToSumMoney + ToSumMoney
