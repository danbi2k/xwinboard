...
SQL = "SELECT DISTINCT gCnt FROM BetGame_TB WHERE ResuFlag='"& Sect &"' Order By gCnt DESC "
...
<%
				...
				SQL = SQL &" b.HcapAllotl, b.vTname, b.vAllot, b.gbTem, convert(varchar(16),b.WriteDay,120 ) As WriteDay, b.bMoney, "
				SQL = SQL &" b.ResuFlag, b.rMoney, b.goldhit, g.rtel "
...
%>		
					...
					<tr height="30" align="center" >
						<td>							<table width="100%" border="0" cellspacing="1" cellpadding="0" bgcolor="#aaaaaa" align=center>
								<tr height="30" align="center" bgcolor="#e0e0e0">
									<td width="90">아이디</td>									<td width="135">베팅일시</td>									<td width="135">경기날짜</td>
									<td width="90">종목</td>
																		<td width="160">승(홈팀)</td>									<td width="75">무/핸드캡</td>
																		<td width="160">패(원정팀)</td>
									<td width="50">배팅팀</td>									<!--<td width="75">경기결과</td>-->
									<td width="140">적중유무</td>									<!--<td width="100">배팅금액</td>									<td width="60">삭제</td>-->
								</tr>									<form  name=frms >
			...
			%>
								...
								<tr height="20" align=center bgcolor="#FFFFFF" onMouseOver='this.style.backgroundColor="#e4e4e4"' onMouseOut='this.style.backgroundColor=""'>													
									...
																		<td align="right"><%=objRs("hTname")%>&nbsp;<%=objRs("hAllot")%>&nbsp;</td>									<td ><%If objRs("gbTem")=3 Then %><%=IfElse(objRs("HcapAllotl"),"0.00")%><%Else%><%=objRs("hCap")%><%End If %></td>									<td align=left>&nbsp;<%=objRs("vAllot")%>&nbsp;<%=objRs("vTname")%></td>									<td ><%If objRs("gbTem")=1 Then %>홈팀<%ElseIf objRs("gbTem")=2 Then%>원정팀<%Else%>무<%End If%></td>									<td ><input type="radio" name="goldhit_<%=objRs("Idx")%>" value=0 onclick="MovePage('<%=objRs("Idx")%>','','GOLD','0','');" <%=gSelection(objRs("goldhit"),0,"checked")%>>미적중&nbsp;<input type="radio" name="goldhit_<%=objRs("Idx")%>" value=1 onclick="MovePage('<%=objRs("Idx")%>','','GOLD','1','');" <%=gSelection(objRs("goldhit"),1,"checked")%>>적중</td>
...
<%
			...
			%>
						...
						배당율 : <%=formatnumber(AllAllot,2)%> / 배팅금액 : <%=formatnumber(betmoney,[])%> / 예상적중금액 : <%=formatnumber(AllhMoney,[])%> / 적중금액 : <input type=text name="rMoney" value="<%=AllrbMoney%>" size=10>									</td>									<td>
