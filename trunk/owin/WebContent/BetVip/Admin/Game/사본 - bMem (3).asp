							...
							<table  border="0" cellspacing="1" cellpadding="0" >
								<tr height="30" >
									...
									<td rowspan=2>
										...
										&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<font color=red>적중금액은 숫자만 입력해 주세요</font>
									</td>
							</table>
						</td>
					</tr>
...
%>		
			...
			%>
								...
								<tr height="20" align=center bgcolor="#FFFFFF" onMouseOver='this.style.backgroundColor="#e4e4e4"' onMouseOut='this.style.backgroundColor=""'>
									...
									<td ><nobr><input type="radio" name="goldhit_<%=objRs("Idx")%>" value=0 onclick="MovePage('<%=objRs("Idx")%>','','GOLD','0','');" <%=gSelection(objRs("goldhit"),0,"checked")%>>대기중&nbsp;<input type="radio" name="goldhit_<%=objRs("Idx")%>" value=2 onclick="MovePage('<%=objRs("Idx")%>','','GOLD','2','');" <%=gSelection(objRs("goldhit"),2,"checked")%>>미적중&nbsp;<input type="radio" name="goldhit_<%=objRs("Idx")%>" value=1 onclick="MovePage('<%=objRs("Idx")%>','','GOLD','1','');" <%=gSelection(objRs("goldhit"),1,"checked")%>>적중</td>
			...
			%>
					...
					<tr>
						...
						<%If Sect=0 Then %>
									...
									<td>
						배당율 : <%=formatnumber(AllAllot,2)%> / 배팅금액 : <%=formatnumber(betmoney,[])%> / 예상적중금액 : <%=formatnumber(AllhMoney,[])%> / 적중금액 : <input type=text name="rMoney" value="<%=AllrbMoney%>" OnKeydown="onlyNumber();" size=10>
									</td>
									<td>
...
%>
		...
		</form>		<tr><td align=center><input type=button value="적중유무 일괄변경" onclick="UpdateGoldhit();"></td></tr>
		<tr>
			<td height=50>
