...
<%
	...
	Sect			= Request("Sect")
	If Sect=5 Then goPage = "List2.asp"
	...
	For i=0 To Ubound(AContent)
		...
		If Not isNone(AContent(i)) Then	
			...
			For b=0 To Ubound(BContent)
				...
				Else
					If objRs(0) <> 1 Then Call CloseDB() : Call descment("이미 마감된 게임에 베팅 하셨습니다.","/Game/"&goPage&"?Sect="& Sect,"")
				End If
...
'	MaxGcnt = RandomCode()
	...
	For i=0 To Ubound(AContent)
		...
		If Not isNone(AContent(i)) Then	
			...
			Set objRs = RunSQLReturnRs("SELECT * FROM Game_TB WHERE Idx='"& reString(BContent(0),"sql") &"'")
			If objRs.EOF Then
				 Call CloseDB() : Call descment("존재하지 않는 게임입니다.","/Game/"&goPage&"?Sect="& Sect,"")
			Else
