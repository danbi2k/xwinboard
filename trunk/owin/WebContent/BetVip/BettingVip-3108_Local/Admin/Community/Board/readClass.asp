...
If CONF_USECOMMENT = 1 Then
	SQL = "SELECT num, cname, cComment, WriteDay, Ip, rCnt, Id FROM "& TableName &"_comment WHERE bnum = " & IntIdx & " And Hidden=0 ORDER BY num DESC"
	Set objRs = RunSQLReturnRS(SQL)
		If objRs.EOF or objRs.BOF Then
			arrComCnt = 0		
		Else	
...
'Response.Write sql
Set objRs = RunSQLReturnRS(SQL)
If Not objRs.BOF or Not objRs.EOF Then  pData = objRs.GetRows() : PreIdx = reString(pData(0,0),"text") : PreTitle = reString(pData(1,0),"text")
objRs.Close

'다음글 서치
SQL = "SELECT Idx,Subject FROM "& TableName & WHERE & NWHERE 
Set objRs = RunSQLReturnRS(SQL)
If Not objRs.BOF or Not objRs.EOF Then nData = objRs.GetRows() : NextIdx = reString(nData(0,0),"text") : NextTitle = reString(nData(1,0),"text")
objRs.Close
