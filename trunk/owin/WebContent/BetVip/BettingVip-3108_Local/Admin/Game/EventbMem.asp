...
'	Response.End
	...
	Call descment("","EventbMem.asp?CurPage="& Request("CurPage") &"&CurBlock="& Request("CurBlock") &"&goPage="& Request("CurPage") &"&KeyField="& Request("KeyField")&"&KeyWord="& Request("KeyWord") &"&Idx="& IntIdx &"&Sect="& Request("Sect") ,"")	
End If
...
SQL = "SELECT DISTINCT gCnt FROM BetGame_TB WHERE ResuFlag=0 And Visbled=0 And Event=1 "
'SQL = "SELECT DISTINCT gCnt, Max(Idx) FROM BetGame_TB WHERE ResuFlag=0 And Visbled=0 And Event=1 Group By gCnt Having count(*) = 6 "
...
<script language='javascript'>
	function MovePage(gId, goP, gfl, gSe, gfrm) {
		...
		getQuery["ThisPage"]	= 'EventbMem.asp';
		getQuery["Idx"]			= gId;
		getQuery["cIdx"]		= gSe;
		getQuery["flag"]		= gfl;
		action(gfrm,goP);
...
//		alert(document.frmsgod.goldhits.value);
	...
	function seArch() {
		if(confirm('삭제 하시겠습니까?')) {
			location.href = 'List_Prc.asp?Sect=<%=Sect%>&f_date='+document.all.f_date.value+'&t_date='+document.all.t_date.value+'&flag=BMEMDELETE&ThisPage=EventbMem.asp';
		}
	}
</script>
<body style="margin:0">
			...
			<td height="54" background="/Images/Admin/top2_bg.gif" align="left" valign="middle">
				<table border="0">
					<tr>
						...
						<td><font color="#ffffff" size="4"><b>[이벤트배팅 관리]</b></font></td>
					</tr>
				</table>
			</td>
		</tr>
		...
		<tr>
			<td align="center">
				<table width="100%" border="0" cellspacing="1" cellpadding="0" bgcolor="#aaaaaa" align=center>
					<form name=regfrm method=post action="EventbMem.asp">
...
%>
		...
		<input type=hidden name=ThisPage value="EventbMem.asp">
