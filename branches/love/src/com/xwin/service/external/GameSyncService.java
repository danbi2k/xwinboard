...
public class GameSyncService extends XwinService {
	...
	public void sync() throws Exception {
			...
			for (Game game : gameList) {
				...
				if (dbGame == null) {
					if (game.getType().equals("wdl")) {
						Double winRate = XwinUtil.doubleCut(calcRate(game
								.getWinRate()));
						Double drawRate = XwinUtil.doubleCut(calcRate(game
								.getDrawRate()));
						Double loseRate = XwinUtil.doubleCut(calcRate(game
								.getLoseRate()));
					...
					if (dbGame.getHomeTeam().equals(syncGame.getHomeTeam()) == false)
						note += dateStr + " 홈팀명변경: " + dbGame.getHomeTeam()
								+ " -> " + syncGame.getHomeTeam() + "\n";
					if (dbGame.getAwayTeam().equals(syncGame.getAwayTeam()) == false)
						note += dateStr + " 원정팀명변경: " + dbGame.getAwayTeam()
								+ " -> " + syncGame.getAwayTeam() + "\n";
					if (dbGame.getGameDate().equals(syncGame.getGameDate()) == false)
						note += dateStr
								+ " 경기일시변경: "
								+ XwinUtil.getBoardItemDate(dbGame
										.getGameDate())
								+ " -> "
								+ XwinUtil.getBoardItemDate(syncGame
										.getGameDate()) + "\n";
					...
					if (game.getType().equals("wdl")) {
						...
						if (dbGame.getWinRate().equals(syncGame.getWinRate()) == false)
							note += dateStr + " 승배당변경 : "
									+ dbGame.getWinRateStr() + " -> "
									+ syncGame.getWinRateStr() + "\n";
						if (dbGame.getDrawRate().equals(syncGame.getDrawRate()) == false)
							note += dateStr + " 무배당변경 : "
									+ dbGame.getDrawRateStr() + " -> "
									+ syncGame.getDrawRateStr() + "\n";
						if (dbGame.getLoseRate().equals(syncGame.getLoseRate()) == false)
							note += dateStr + " 패배당변경 : "
									+ dbGame.getLoseRateStr() + " -> "
									+ syncGame.getLoseRateStr() + "\n";
					} else if (game.getType().equals("handy")) {
						Double winRate = XwinUtil
								.doubleCut(game.getWinRate() + 0.02);
						Double loseRate = XwinUtil
								.doubleCut(game.getLoseRate() + 0.02);
						Double drawRate = XwinUtil
								.doubleCut(game.getDrawRate());
		...
		if (processList != null) {
			for (Game game : processList) {
				...
				if (dbGame != null
						&& dbGame.getStatus().equals(Code.GAME_STATUS_RUN)) {
	...
	public void leagueSync() throws Exception
	{
		String leagueXml = null;
		HttpClient hc = new HttpClient();
		hc.getHttpConnectionManager().getParams().setSoTimeout(50000);
		
		PostMethod post = new PostMethod(syncUrl + "/protocol.php?mode=getLeagueList");		
		post.addRequestHeader("Content-type", "application/x-www-form-urlencoded; charset=utf-8");
		
		try {	
			hc.executeMethod(post);
			leagueXml = post.getResponseBodyAsString();
			System.out.println(leagueXml);
		} catch (Exception e) {
			e.printStackTrace();
			throw e;
		}
		
		List<League> leagueList = (List<League>) XmlUtil.fromXml(leagueXml);
		System.out.println(leagueXml);

		if (leagueList != null) {
			leagueDao.removeAllLeague();
			hc = new HttpClient();
			hc.getHttpConnectionManager().getParams().setSoTimeout(50000);
			for (League league : leagueList) {
				leagueDao.insertSyncLeague(league);
				
				File file = new File(Code.LEAGUE_IMAGE_PATH + File.separator + league.getImage());
				System.out.println(file.getAbsolutePath());
				if (file.exists()) {
					System.out.println(league.getImage() + " EXIST");
				} else {
					post = new PostMethod(syncUrl + "/images/league/" + league.getImage());
					try {	
						hc.executeMethod(post);
						...
						System.out.println(league.getImage() + " DOWNLOADED");
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
