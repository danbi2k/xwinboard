...
//--------------------------------------------------------------------------------------------------------
public class WapGatewayFilter implements Filter
{
			...
			FilterChain filterChain) throws IOException, ServletException
	{
		...
		if (redirect != null) {
			...
			if (token != null && token.length() > 0) {
				Member sessionMember = getSessionMember(token, phoneno);
				request.setAttribute("Member", sessionMember);
			} else {
				Member adhocMember = selectValidMemberByMobile(phoneno);
				if (adhocMember != null)
					request.setAttribute("adhocMember",adhocMember);
			}
			filterChain.doFilter(request, response);
		} else {
			response.sendRedirect("error_device.jsp");
		}
	}
	...
	protected Member selectValidMemberByMobile(String mobile) {
		...
		param.put("mobile", formetedMobile);
	...
	protected Member getSessionMember(String token, String phoneNumber)
	{
		Member sessionMember = null;
		if (StringUtils.isNumeric(token)) {
			long today = Calendar.getInstance().getTimeInMillis();
			long tokenDay = Long.parseLong(token);
			
			if (today - tokenDay < ONE_HOUR) {
				sessionMember = selectValidMemberByMobile(phoneNumber);
			}			
		}	
